...
const { reviewPR, REVIEW_MODEL, REVIEW_EFFORT } = require("./lib/reviewer");
...
function enqueueReview(pr) {
  ...
  semaphore
    .run(async () => {
      ...
      persistReviews();
      log(`reviewing ${key} (${REVIEW_MODEL}/${REVIEW_EFFORT})…`);
...
const server = app.listen(PORT, () => {
  ...
  log(`reviewing with claude "${REVIEW_MODEL}" (effort ${REVIEW_EFFORT}), ${REVIEW_CONCURRENCY} at a time`);
