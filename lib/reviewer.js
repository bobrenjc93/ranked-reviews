  ...
  return {
    ...
    comments: comments
      ...
      .map((c) => ({
        ...
        severity: ["blocker", "warning", "nit"].includes(c.severity) ? c.severity : "nit",
...
// Human-friendly model label (e.g. "opus 4.8"), resolved from the model id
// `claude` actually runs rather than the alias we pass in. Cached once known.
let modelLabel = null;
...
// `modelUsage`. Cache the first one we see so log lines can show the version.
...
// The version-qualified model label if known, else the alias we were given.
function modelDisplay() {
  return modelLabel || REVIEW_MODEL;
}

// Resolve the model version up front (a tiny throwaway call) so the very first
// review log line already shows the version. Best-effort: falls back to the
// alias if claude is slow or unavailable.
async function resolveModel() {
  if (modelLabel) return modelDisplay();
  try {
    const { stdout } = await run(
      "claude",
      ["-p", "ok", "--output-format", "json", "--model", REVIEW_MODEL],
      { cwd: os.tmpdir(), timeout: 30_000 }
    );
    rememberModel(parseClaudeEnvelope(stdout));
  } catch {
    // best effort; log lines will show the alias until a real review resolves it
  }
  return modelDisplay();
}
...
module.exports = { reviewPR, resolveModel, modelDisplay, REVIEW_MODEL, REVIEW_EFFORT };
